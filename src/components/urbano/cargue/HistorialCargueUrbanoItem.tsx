...
export const HistorialCargueUrbanoItem = () => {

    const { state } = useOperativosContext();

    const operativoFiltered = state.carguesUrbanos.filter(operativo => (Date.parse(operativo.fechaHoraIngreso.split(',')[0])) === state.filtro.fecha && !operativo.enBodega)
                ...
                <>
                    {
                        operativoFiltered.map(operativo => (
                            <div key={operativo.id} className='p-5 shadow-lg rounded-lg'>
                                ...
                                <p><strong>Plantilla:</strong>  {operativo.plantilla}</p>
                                <p><strong>Asignación:</strong>  {operativo.asignacion}</p>
